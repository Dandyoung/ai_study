...
def generate_sql_query(question, table_info, table_name):
    prompt = f"""
    주어진 질문을 올바른 SQL 쿼리로 변환해주세요.
    추가적인 설명이나 주석 없이 SQL 쿼리만 작성해주세요.
    반복적인 메시지나 문구를 사용하지마세요.

    아래는 예시를 위한 {table_name}의 열과 3가지 행입니다 : 
    {table_info}.
    ...
    response = text_generation_pipeline(prompt, max_length=2000, 
                                        do_sample=True, 
...
# SQL 쿼리에서 ```으로 감싸진 부분 추출
preprocessing_query = re.search(r'```(.*?)```', sql_query, re.DOTALL)
if preprocessing_query:
    preprocessing_query = preprocessing_query.group(1).strip()

# 결과를 JSON 형식으로 저장
output = {"response": sql_query, "preprocessing": preprocessing_query}
